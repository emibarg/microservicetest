...
func (l *Login) Login(context *gin.Context) {
	...
	ok(context, http.StatusOK, "token generated", tokenString)
}
...
func (l *Login) VerifyToken(context *gin.Context) {
	...
	if !valid {
		badRequest(context, http.StatusUnauthorized, "invalid token", nil)
		return
	}
	ok(context, http.StatusOK, "token verified", claims)
